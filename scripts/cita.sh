...
do_start() {
    ...
    if [ -z "${debug}" ]; then
        ...
        RUST_LOG=cita_vm=${debug},                                                                                                                                                                                                                                              cita_auth=${debug},cita_chain=${debug},cita_executor=${debug},cita_jsonrpc=${debug},cita_network=${debug},cita_bft=${debug},\
core_executor=${debug},engine=${debug},jsonrpc_types=${debug},libproto=${debug},proof=${debug},txpool=${debug},core=${debug},cita_relayer_parser=${debug} \
        cita-forever -c "${config}" start 2>&1
    fi
